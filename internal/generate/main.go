...
func generateGoFunctions(filenames ...string) {
	for _, inpath := range filenames {
		fs := token.NewFileSet()
		file, err := parser.ParseFile(fs, filepath.Join("internal", inpath), nil, parser.ParseComments)
		if err != nil {
			panic(err)
		}
		...
		code.WriteString("\t\"github.com/judah-caruso/raylib-purego/internal\"\n")
		code.WriteString("\t\"unsafe\"\n")
		code.WriteString(")\n\n")
		...
		ast.Inspect(file, func(n ast.Node) bool {
			...
			for _, decl := range file.Decls {
				...
				for _, spec := range d.Specs {
					...
					bindCall := val.Values[0].(*ast.CallExpr)

					goName := bindCall.Args[1].(*ast.BasicLit).Value
					goName = goName[1 : len(goName)-1]

					var cArgs []carg
					if len(bindCall.Args) > 2 {
						for i, arg := range bindCall.Args[2:] {
							var name string
			...
			for _, proc := range procs {
				...
				if indirects := strings.Count(resGoType, "Ptr"); indirects != 0 {
					resGoType = strings.ReplaceAll(resGoType, "Ptr", "")
					resGoType = strings.Repeat("*", indirects) + resGoType
				}
				...
				for i, arg := range proc.CArgs {
					...
					if indirects := strings.Count(goType, "Ptr"); indirects != 0 {
						goType = strings.ReplaceAll(goType, "Ptr", "")
						goType = strings.Repeat("*", indirects) + goType
					}
				...
				case "void", "":
					voidResult = true
					fmt.Fprintf(&body, "\tinternal.%s.Void", proc.CName)
				case "bool":
					fmt.Fprintf(&body, "\tres = internal.%s.Bool", proc.CName)

				case "uint8", "uint16", "uint32", "uint64",
					"int8", "int16", "int32", "int64":
					procName := strings.ToUpper(resGoType[0:1]) + resGoType[1:]
					fmt.Fprintf(&body, "\tres = internal.%s.%s", proc.CName, procName)

				default:
					needsPointer = true
					fmt.Fprintf(&body, "\tinternal.%s.Call", proc.CName)
				}
...
func generateFFITypes() {
	const inpath = "raylib.go"
	...
	var code strings.Builder
	code.WriteString("// Code generated by go generate; DO NOT EDIT.\n\n")
	code.WriteString("package internal\n\n")
	...
	if err := os.WriteFile(filepath.Join("internal", "ffi_types.go"), src, 0664); err != nil {
		panic(err)
	}
}
...
func generateInitFile(filename string, libpath string, gobuild string) {
	...
	fmt.Fprintf(&code, "const libraryName = %q\n\n", libpath)

	fmt.Fprintf(&code, "// The data of %s\n", libpath)
	code.WriteString("const libraryData = \"")
	...
	outpath := filepath.Join("internal", "lib_"+filename)
	if err := os.WriteFile(outpath, []byte(code.String()), 0664); err != nil {
		panic(err)
	}
}
